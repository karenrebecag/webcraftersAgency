...
(function ($) {
    ...
    $(document).ready(function () {
        ...
        gsap.utils.toArray('.wrapper').forEach((wrapper, index) => {
            const [x, xEnd] = (index % 2) ? ['100%', (wrapper.scrollWidth - wrapper.offsetWidth) * -1] : [wrapper.scrollWidth * -1, 0];
          
            gsap.fromTo(wrapper, { x }, {
              x: xEnd,
              ease: 'power2.inOut', // Agrega un ease más suave
              scrollTrigger: {
                trigger: '.demo-section',
                start: 'top 80%', // La animación comienza cuando la sección ha entrado un 20%
                end: 'bottom 20%', // La animación termina cuando la sección ha salido un 20%
                scrub: 0.5
              }
            });
          });
